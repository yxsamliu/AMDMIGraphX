...
struct find_dot_add
{
    auto matcher() const
    {
        return match::name("dot")(match::nargs(3));
    }
    ...
    void apply(program& p, match::matcher_result r) const
    {
        ...
        if (not float_equal(dot.beta, 1))
            return;
        auto dot_ins = p.insert_instruction(ins, op::dot{dot.alpha, 0}, ins->inputs()[0], ins->inputs()[1]);
...
{
    match::find_matches(p, find_dot_add{});
}
