    ...
    for(auto ins : iterator_for(p))
    {
        ...
        visit_all(gamma, bias, mean, variance, a, b)(
            [&](auto gamma2, auto bias2, auto mean2, auto variance2, auto a2, auto b2) {
                dfor(a.get_shape().elements())([&](std::size_t c) {
                    a2[c] = double(gamma2[c]) / std::sqrt(double(variance2[c]) + epsilon);
                });
                dfor(b.get_shape().elements())([&](std::size_t c) {
                    b2[c] = double(bias2[c]) - (double(gamma2[c]) * double(mean2[c]) /
                                                std::sqrt(double(variance2[c]) + epsilon));
                });
            });
