                ...
                std::vector<std::size_t> offsets)
{
    ...
    {
        auto nelements = std::max_element(args.begin(), args.end(), [](auto x, auto y) {
            return x.get_shape().elements() < y.get_shape().elements();
        })->get_shape().elements();
        visit_concat_sizes(ninputs, [&](auto n) {
            ...
            hip_visit_all(args.back(), dargs, arg_shapes)([&](auto output, auto inputs, auto input_shapes) {
