...
TEST_CASE(load_and_run_var_default)
{
    auto p             = migraphx::parse_onnx("crm_var_len.onnx");
    auto shapes_before = p.get_output_shapes();
    p.compile(migraphx::target("cpu"));
    auto shapes_after = p.get_output_shapes();
    CHECK(shapes_before.size() == 1);
...
TEST_CASE(load_and_run_var_lens_2)
{
    migraphx::onnx_options options;
    options.add_dim_param_value("batch", 2);
    auto p             = migraphx::parse_onnx("crm_var_len.onnx", options);
    auto shapes_before = p.get_output_shapes();
    p.compile(migraphx::target("cpu"));
    auto shapes_after = p.get_output_shapes();
    CHECK(shapes_before.size() == 1);
